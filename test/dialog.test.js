...
const run = () => {
  ...
  assert.equal(resolveSessionName('backup', prefix), 'backup')
  assert.equal(resolveSessionName('a-z_', prefix), 'a-z_')
  assert.equal(resolveSessionName('v2', prefix), 'v2')
  assert.equal(resolveSessionName(`${prefix}-2026`, prefix), `${prefix}-2026`)
  assert.equal(resolveSessionName('bad name', prefix), null)
  assert.equal(resolveSessionName('', prefix), prefix)
